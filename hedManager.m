classdef hedManager
    ...
    % (a) Matching an input HED tag or string to a provided XML-formatted HED specification.
    % (b) Match a HED string to another HED string: whether an event described by the input HED string can be
    %     associated with second string. For example, the HEd string (Stimulus/Visual/Circle,
    %     Stimulus/Auditory, /Response/Button) matches (Stimulus/Visual, Stimulus/Auditory) but not
    %     vice verse.
    % (c) Auto-complete partial HED tag, for example /Stimulus/Vi brings up all tags at that level
    %     (here: /Stimulus/Visual)
    %  
    % written by Nima Bigdely Shamlo, 2013
    properties
        hedNode
        hedVersion
    end;
    ...
    methods
        function [answer, description, levelIsValid, levelMatchedWithHEDNode, autoCompleteAlternative, requireChild] = isNodeSequenceValid(obj, nodeSequence, varargin)
            ...
            requireChild = false(1, length(nodeSequence));
            ...
            if currentNode.hasChildNodes
                for currentLevel = 1:length(nodeSequence)
                    ...
                    % the node that only has one child must only contain some text and no children.
                    ...
                    for i=1:children.getLength
                        ...
                        if children.item(i-1).hasChildNodes
                            ...
                            if strcmpi('node', char(children.item(i-1).getNodeName))
                                ...
                                % find the 'name' node inside the 'node' node (due to poetntial
                                % newline and enter chasractersm this might not be the first node).
                                potentialNameNodeArray = children.item(i-1).getElementsByTagName('name');                                               
                                nameNode = [];
                                for nameCounter=1:potentialNameNodeArray.getLength
                                    if potentialNameNodeArray.item(nameCounter-1).getParentNode.isEqualNode(children.item(i-1)),
                                        nameNode = potentialNameNodeArray.item(nameCounter-1);
                                    end;
                                end;
                                
                                if ~isempty(nameNode)                                    
                                    originalTextContent = strtrim(char(nameNode.getFirstChild.getData));
                        ...
                        if  nargout > 3 && currentLevel == length(nodeSequence) && ~isempty(textContent) && any((strncmpi(nodeSequence{currentLevel}, textContent, length(nodeSequence{currentLevel}))))
                            autoCompleteAlternative{end+1} = originalTextContent;
                        end;
                        
                        % if there is a match, prevent matching to an empty node
                        exactMatch = any(strcmpi(textContent, nodeSequence{currentLevel})) && ~isempty(nodeSequence{currentLevel});                                                
                        ...
                        if exactMatch
                            ...
                            requireChild(currentLevel) = currentNode.hasAttribute('requireChild');                                              
                            levelIsValid(currentLevel) = true;
    ...
    methods
        ...
        function obj = hedManager(xmlFilePath, varargin)                        
            ...
            if nargin < 1
                ...
                xmlFilePath  = [fullPath(1:end - length('hedManager.m')) 'Hed Specification 1.3.xml'];
            end;
        ...
        function [answer, description, levelIsValid, levelMatchedWithHEDNode, nodeSequence, requireChild] = isValidHedTag(obj, hedTag, varargin)
           ...
           %                           availabe in HED xml (inside <description> tags).
            ...
            [answer, description, levelIsValid, levelMatchedWithHEDNode, ~, requireChild] = isNodeSequenceValid(obj, nodeSequence);
        ...
        function [answer, nodeSequence, requireChild] = isValidHedString(obj, hedString, varargin)
            ...
            requireChild = cell(length(hedTag),1);
            nodeSequence = cell(length(hedTag),1);
            
            for i=1:length(hedTag)
                [answerForTag(i), description, levelIsValid, levelMatchedWithHEDNode, nodeSequence{i}, requireChild{i}] = isValidHedTag(obj, char(strtrim(hedTag{i})));
            end;
